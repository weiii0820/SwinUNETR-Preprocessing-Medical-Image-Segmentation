...
def create_datalist_from_folders(data_dir: str):
    ...
    label_paths = sorted(glob.glob(os.path.join(data_dir, "labels", "*.nii")))
    datalist = []
...
def remap_labels_func(label_tensor):
    ...
    lookup_table = torch.arange(27, dtype=torch.long, device=device)
...
model = SwinUNETR(img_size=IMG_SIZE, in_channels=IN_CHANNELS, out_channels=OUT_CHANNELS, feature_size=FEATURE_SIZE, use_checkpoint=False,).to(DEVICE)
# 損失函數會自動處理 21 個類別
